...
exports.trackReservationResult = trackReservationResult;
const ALLOWED_POLICY = { requiresConfirmation: false, mustHandoff: false };
// ── Helpers ────────────────────────────────────────────────────────────────────
...
function lastMessageIsConfirmation(session) {
    return /\b(yes|yeah|yep|confirm|go ahead|proceed|ok|okay|sure)\b/i.test(lastUserMessage(session));
}
const TOOL_GATES = {
    // ── book_taxi ───────────────────────────────────────────────────────────────
    book_taxi: (args, session) => {
        ...
        return {
            ...
            mustHandoff: false,
            confirmationQuestion: `Just to confirm — shall I book a taxi to ${dest} (quote ${quote})? Please say yes or no.`,
    ...
    check_out: (args, session) => {
        ...
        return {
            requiresConfirmation: true,
            mustHandoff: false,
            confirmationQuestion: `To confirm — shall I proceed with checking out reservation ${resId}? ` +
    ...
    create_reservation: (args, session) => {
        ...
        if (guestName.split(/\s+/).filter(Boolean).length < 2) {
            return {
                requiresConfirmation: true,
                mustHandoff: false,
                confirmationQuestion: "Could I have the guest's full name — first and last name please?",
        ...
        if (!contact) {
            return {
                requiresConfirmation: true,
                mustHandoff: false,
                confirmationQuestion: "Could I take a contact phone number or email address for the booking?",
        ...
        if (!lastMessageIsConfirmation(session)) {
            ...
            return {
                requiresConfirmation: true,
                mustHandoff: false,
                confirmationQuestion: `Please confirm the following booking:\n` +
                    `  • Guest: ${guestName}\n` +
                    `  • Room: ${roomType}\n` +
 ...
 */
function checkPolicy(action, session) {
    ...
    if (action.tool === 'find_reservation' && session.reservationNotFoundCount >= 2) {
        return {
            requiresConfirmation: false,
            mustHandoff: true,
            handoffReason: 'Reservation not found after multiple attempts. Escalating to front desk staff.',
 ...
 */
function trackReservationResult(session, result) {
    const res = result;
