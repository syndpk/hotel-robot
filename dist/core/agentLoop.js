...
function buildSystemPrompt(context) {
    ...
    const conciergeText = conciergeSnippets.length > 0
        ? conciergeSnippets.map((s) => `[Source: ${s.sourceId}]\n${s.text}`).join('\n\n')
        : '(No concierge snippets retrieved.)';
    const toolSchemas = (0, registry_1.buildToolSchemasText)();
    ...
    let identitySection = '';
    const identity = context.slots['identity'];
    if (identity) {
        const f = identity.maskedFields;
        const lines = [];
        if (f.fullName)
            lines.push(`Full Name:        ${f.fullName}`);
        if (f.documentType)
            lines.push(`Document Type:    ${f.documentType}`);
        if (f.nationality)
            lines.push(`Nationality:      ${f.nationality}`);
        if (f.dateOfBirth)
            lines.push(`Date of Birth:    ${f.dateOfBirth}`);
        if (f.expiryDate)
            lines.push(`Expiry Date:      ${f.expiryDate}`);
        if (f.issuingCountry)
            lines.push(`Issuing Country:  ${f.issuingCountry}`);
        if (f.documentNumber)
            lines.push(`Doc Number:       ${f.documentNumber} (masked)`);
        if (f.mrzMaskedLast6)
            lines.push(`MRZ Last-2:       ${f.mrzMaskedLast6}`);
 ...
 */
async function proposeAndVerify(baseMessages, llm, session, emit) {
    ...
    for (let correction = 0; correction <= MAX_CORRECTIONS; correction++) {
        ...
        if (extracted === null) {
            ...
            messages.push({
                role: 'user',
                content: 'Your response was not valid JSON. Please output ONLY the JSON object described in the system prompt. No extra text, no markdown fences.',
            });
            continue;
        }
        const validation = types_1.ProposalSchema.safeParse(extracted);
        if (!validation.success) {
            ...
            messages.push({
                role: 'user',
                content: `Your JSON does not match the required Proposal schema. Errors:\n${validation.error.issues.map((i) => `- ${i.path.join('.')}: ${i.message}`).join('\n')}\n\nPlease output a corrected Proposal JSON.`,
            });
            continue;
        }
        ...
        // ── Verify ────────────────────────────────────────────────────────────────
        const verifyResult = (0, verifier_1.verifyProposal)(proposal, session);
        ...
        if ((0, verifier_1.hasFatalViolation)(verifyResult)) {
        ...
        if (correction < MAX_CORRECTIONS) {
            ...
            messages.push({
                role: 'user',
                content: `Your Proposal has ${verifyResult.violations.length} issue(s) that must be fixed:\n\n${correctionItems}\n\nPlease output a corrected Proposal JSON that addresses ALL of the above.`,
            });
        }
    }
...
async function runAgentLoop(context, session, userMessage, opts) {
    const llm = (0, llm_1.getLLMClient)();
    const stepHistory = [];
    let stepIndex = 0;
    function emit(event, details, durationMs) {
        const step = { stepIndex, timestamp: now(), event, details, durationMs };
        opts.onTrace(step);
    }
    ...
    for (stepIndex = 0; stepIndex < MAX_STEPS; stepIndex++) {
            ...
            const lines = stepHistory.map((s, i) => {
                ...
                if (action.action === 'CALL_TOOL') {
                    ...
                    if (s.toolResult !== undefined)
                        line += ` → result=${JSON.stringify(s.toolResult).slice(0, 400)}`;
                    if (s.toolError)
                        line += ` → ERROR: ${s.toolError}`;
                }
                return line;
            });
            toolResultsContext =
        ...
        {
            const idSlot = context.slots['identity'];
        ...
        if (!result) {
            ...
            return (`I'm connecting you with our front desk team right away.\n\n` +
                `Please call us at ${hotel_1.HOTEL_CONFIG.phone} or visit reception.`);
        }
        ...
        if (handoffAction && handoffAction.type === 'handoff') {
            ...
            return (`${proposal.assistant_message}\n\n` +
                `Please call us at ${hotel_1.HOTEL_CONFIG.phone} or visit the front desk.`);
        }
        // ── Execute tool_calls ───────────────────────────────────────────────────
        ...
        for (const toolAction of toolCalls) {
            if (toolAction.type !== 'tool_call')
                continue;
            const t0 = Date.now();
            let toolResult;
            try {
                toolResult = await (0, registry_1.executeTool)(toolAction.tool, toolAction.args);
            }
            catch (err) {
                ...
                const fakeAction = { action: 'CALL_TOOL', tool: toolAction.tool, args: toolAction.args };
    ...
    return (`I apologise — I was unable to complete your request within the allowed steps. ` +
        `Please speak with our front desk team directly, or call us at ${hotel_1.HOTEL_CONFIG.phone}.`);
}
// ── Session state updater ──────────────────────────────────────────────────────
function updateSession(session, tool, result) {
    const res = result;
    switch (tool) {
        case 'find_reservation':
            (0, policy_1.trackReservationResult)(session, result);
            if (Array.isArray(res['matches']) && res['matches'].length > 0) {
                const first = res['matches'][0];
                session.lastReservationId = first['reservationId'];
        ...
        case 'create_reservation':
            if (res['reservationId']) {
                session.lastReservationId = res['reservationId'];
        ...
        case 'book_taxi':
            if (res['bookingId']) {
                session.lastBookingId = res['bookingId'];
        ...
        case 'check_out':
            if (res['roomNumber'])
                session.slots['roomNumber'] = res['roomNumber'];
            break;
    }
