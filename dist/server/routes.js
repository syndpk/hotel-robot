...
const passport_1 = require("../identity/passport");
// ── Route registration ─────────────────────────────────────────────────────────
async function registerRoutes(app) {
    // ── POST /api/session ──────────────────────────────────────────────────────
    app.post('/api/session', async (_req, reply) => {
        const session = sessionStore_1.sessionStore.createSession();
    ...
    app.post('/api/message', async (req, reply) => {
        const { sessionId, text, voice = false } = req.body;
        ...
        const run = sessionStore_1.sessionStore.createRun(sessionId, text.trim());
        // Fire-and-forget — pass runId so orchestrator reuses this run (no duplicate)
        setImmediate(() => {
            orchestrator_1.orchestrator.process({ sessionId, input: text.trim(), voiceEnabled: voice, runId: run.runId }).catch((err) => {
    ...
    app.post('/api/audio', async (req, reply) => {
        ...
        let transcript;
        try {
            const sttResult = await (0, elevenlabs_1.transcribeAudio)(audioBuffer, mimeType, langHint);
        ...
        const run = sessionStore_1.sessionStore.createRun(sessionId, transcript);
        setImmediate(() => {
            orchestrator_1.orchestrator.process({
                sessionId,
                input: transcript,
    ...
    app.get('/api/runs/:runId', async (req, reply) => {
        ...
        const run = sessionStore_1.sessionStore.getRun(runId);
        if (!run) {
    ...
    app.get('/api/runs/:runId/audio', async (req, reply) => {
        const { runId } = req.params;
        const run = sessionStore_1.sessionStore.getRun(runId);
        if (!run)
            return reply.code(404).send({ error: 'Run not found.' });
        if (!run.audioOutputBase64)
            return reply.code(404).send({ error: 'No audio for this run.' });
    ...
    app.get('/api/runs/:runId/events', (req, reply) => {
        ...
        const existingRun = sessionStore_1.sessionStore.getRun(runId);
        if (existingRun) {
        ...
        const onStep = (step) => sendStep(step);
        const onDone = (payload) => {
            sendEvent('done', payload);
            cleanup();
            res.end();
        };
        orchestrator_1.runEventBus.on(`step:${runId}`, onStep);
        orchestrator_1.runEventBus.once(`done:${runId}`, onDone);
        ...
        function cleanup() {
            ...
            orchestrator_1.runEventBus.off(`step:${runId}`, onStep);
            orchestrator_1.runEventBus.off(`done:${runId}`, onDone);
        }
    ...
    app.post('/api/passport', async (req, reply) => {
        const data = await req.file();
        if (!data) {
        ...
        let parseResult;
        try {
            parseResult = await (0, passport_1.parseDocument)(imageBuffer);
        }
        catch (err) {
        ...
        sessionStore_1.sessionStore.updateSession(sessionId, { slots: session.slots });
        return reply.send({
            ok: parseResult.ok,
            maskedFields: parseResult.maskedFields,
    ...
    app.post('/api/passport/confirm', async (req, reply) => {
    ...
    app.get('/api/health', async (_req, reply) => {
        const elevenlabs = await (0, elevenlabs_1.checkElevenLabs)();
