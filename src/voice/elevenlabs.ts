...
export async function transcribeAudio(
  ...
  if (data.language_code && data.language_code !== lang) {
    ...
    console.warn(
      `[elevenlabs] STT detected "${data.language_code}" but hint was "${lang}". ` +
 ...
 */
export async function synthesiseSpeechBase64(text: string): Promise<string> {
 ...
 */
export async function checkElevenLabs(): Promise<ElevenLabsHealthResult> {
  const key = process.env.ELEVENLABS_API_KEY;
