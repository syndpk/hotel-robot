...
export interface IdentityMaskedFields {
  ...
  documentType?:   string;  // 'PASSPORT' | 'ID_CARD' | 'VISA' | 'UNKNOWN'
  sex?:            string;
}
...
function countryCode(code: string | null | undefined): string | undefined {
  if (!code) return undefined;
  return code.replace(/<+$/, '').trim() || undefined;
}

function docType(format: string): string {
  switch (format) {
 ...
 */
function mrzFingerprint(num: string): string {
  const cleaned = num.replace(/<+$/, '');
  if (cleaned.length < 2) return '******';
  return '****' + cleaned.slice(-2);
}

function maskFields(raw: IdentityRawFields): IdentityMaskedFields {
 ...
 */
function detectMRZLines(ocrText: string): string[] | null {
 ...
 */
function heuristicExtract(fullOCRText: string): Partial<IdentityRawFields> {
  const lines = fullOCRText.split(/\r?\n/).map(l => l.trim()).filter(Boolean);
  const result: Partial<IdentityRawFields> = {};

  // Surname / Given name: lines that look like all-caps names
  const nameLines = lines.filter(l => /^[A-Z][A-Z\s\-]{3,}$/.test(l));
  if (nameLines.length >= 1) result.surname = nameLines[0];
  if (nameLines.length >= 2) result.givenNames = nameLines[1];
  if (result.surname) {
  ...
  if (dates.length >= 1 && dates[0]) result.dateOfBirth = dates[0].replace(/[./]/g, '-');
  if (dates.length >= 2 && dates[1]) result.expiryDate  = dates[1].replace(/[./]/g, '-');
...
const ZERO_CONF: IdentityConfidences = {
 ...
 */
export async function parseDocument(imageBuffer: Buffer): Promise<ParseResult> {
  const errors: string[] = [];

  // ── 1. Preprocess full image ───────────────────────────────────────────────
  let processedFull: Buffer;
  let mrzBuffer: Buffer;
  ...
  try {
    ...
    processedFull = await sharp(imageBuffer)
      .grayscale()
      .normalise()
      .sharpen()
      .toBuffer();
    ...
    mrzBuffer = await sharp(imageBuffer)
      .grayscale()
      .normalise()
      .sharpen()
  ...
  // ── 2. OCR the MRZ zone ────────────────────────────────────────────────────
  let mrzOCRText = '';
  let mrzWorker: Awaited<ReturnType<typeof Tesseract.createWorker>> | null = null;
  try {
    mrzWorker = await Tesseract.createWorker('eng', 1, { logger: () => {} });
    ...
    if (mrzWorker) await mrzWorker.terminate().catch(() => {});
  }
  ...
  if (mrzLines) {
    ...
    if (mrzResult) {
      const f = mrzResult.fields as Record<string, string | null>;
      const fmt = mrzResult.format as string;

      // Find failed check digits from details
      const failedChecksums: string[] = [];
      if (Array.isArray(mrzResult.details)) {
        for (const d of mrzResult.details as Array<{ field: string; valid: boolean; type?: string }>) {
      ...
      const rawFields: IdentityRawFields = {
        fullName,
      ...
      return {
        ...
        validation: {
          mrzFound: true,
          mrzFormat: fmt as 'TD1' | 'TD2' | 'TD3' | 'MRVA' | 'MRVB',
  ...
  let fullOCRText = '';
  let fullWorker: Awaited<ReturnType<typeof Tesseract.createWorker>> | null = null;
  try {
    fullWorker = await Tesseract.createWorker('eng', 1, { logger: () => {} });
    const { data } = await fullWorker.recognize(processedFull);
    fullOCRText = data.text ?? '';
  } catch {
    errors.push('FULL_IMAGE_OCR_FAILED');
  } finally {
    if (fullWorker) await fullWorker.terminate().catch(() => {});
  }
  ...
  const rawFields: IdentityRawFields = { ...heuristic, documentType: 'UNKNOWN' };
  const lowConf = 0.35;
  const conf: IdentityConfidences = {
